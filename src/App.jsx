...
import { supabase, TRIPS_TABLE, STYLE_ROW_ID } from "./supabaseClient";
import {
  MapPin, Plus, X, ChevronDown, ChevronUp, ArrowLeft, Search, GripVertical, Flag, Settings, Mail, LogOut, Download, Upload,
...
function PostmarkApp({ session }) {
  ...
  const prevSyncedStyleRef = useRef(null);
  const restoreFileRef = useRef(null);

  // Initial load: pull from Supabase (source of truth once logged in). If the
  // cloud has nothing yet but this device has local data, push it up rather
  // than discarding it. Falls back to the local cache if the cloud is
  // unreachable, so the app still works offline.
  useEffect(() => {
    let cancelled = false;
    ...
    async function init() {
      ...
      try {
        ...
        localTrips = rawTrips ? JSON.parse(rawTrips).map(migrateTrip) : null;
      } catch (e) { /* ignore */ }
      try {
        const rawStyle = localStorage.getItem(STYLE_STORAGE_KEY);
        localStyle = rawStyle ? JSON.parse(rawStyle) : null;
      } catch (e) { /* ignore */ }

      const { data, error } = await supabase.from(TRIPS_TABLE).select("id, data").eq("user_id", session.user.id);
      if (cancelled) return;

      if (error) {
        console.error("Postmark: could not reach cloud storage, using local cache", error);
        setTrips(localTrips && localTrips.length ? localTrips : [buildSeedTrip()]);
        setStyleSettings(localStyle ? { ...DEFAULT_STYLE_SETTINGS, ...localStyle } : DEFAULT_STYLE_SETTINGS);
        setLoaded(true);
        setStyleLoaded(true);
        setCloudReady(false);
        setCloudError(true);
      ...
      const styleRow = (data || []).find((r) => r.id === STYLE_ROW_ID);
      const cloudTrips = (data || []).filter((r) => r.id !== STYLE_ROW_ID).map((r) => migrateTrip(r.data));

      let finalTrips;
      if (cloudTrips.length > 0) {
        finalTrips = cloudTrips;
      } else if (localTrips && localTrips.length > 0) {
        finalTrips = localTrips;
        const rows = localTrips.map((t) => ({ id: t.id, user_id: session.user.id, data: t }));
        const { error: pushErr } = await supabase.from(TRIPS_TABLE).upsert(rows, { onConflict: "user_id,id" });
        if (pushErr) console.error("Postmark: failed to push local trips to cloud", pushErr);
      } else {
        const seed = buildSeedTrip();
        finalTrips = [seed];
        const { error: seedErr } = await supabase.from(TRIPS_TABLE).upsert([{ id: seed.id, user_id: session.user.id, data: seed }], { onConflict: "user_id,id" });
        if (seedErr) console.error("Postmark: failed to save starter trip to cloud", seedErr);
      }

      let finalStyle = DEFAULT_STYLE_SETTINGS;
      if (styleRow && styleRow.data) {
        finalStyle = { ...DEFAULT_STYLE_SETTINGS, ...styleRow.data };
      } else if (localStyle) {
        finalStyle = { ...DEFAULT_STYLE_SETTINGS, ...localStyle };
        await supabase.from(TRIPS_TABLE).upsert([{ id: STYLE_ROW_ID, user_id: session.user.id, data: finalStyle }], { onConflict: "user_id,id" });
      }

      const snapshot = {};
      finalTrips.forEach((t) => { snapshot[t.id] = JSON.stringify(t); });
      prevSyncedRef.current = snapshot;
      prevSyncedStyleRef.current = JSON.stringify(finalStyle);

      if (cancelled) return;
      setTrips(finalTrips);
      setStyleSettings(finalStyle);
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(finalTrips)); } catch (e) { /* ignore */ }
      try { localStorage.setItem(STYLE_STORAGE_KEY, JSON.stringify(finalStyle)); } catch (e) { /* ignore */ }
      setLoaded(true);
      setStyleLoaded(true);
      setCloudReady(true);
    }
  ...
  // Local cache stays in sync for speed/offline use; once the cloud is ready,
  // any change to a trip (or a whole-trip delete) gets pushed up too.
  useEffect(() => {
    if (!loaded || trips === null) return;
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(trips)); } catch (e) { /* ignore */ }
    if (!cloudReady) return;

    const prevMap = prevSyncedRef.current || {};
    const nextMap = {};
    const upserts = [];
    trips.forEach((t) => {
      const json = JSON.stringify(t);
      nextMap[t.id] = json;
      if (prevMap[t.id] !== json) upserts.push({ id: t.id, user_id: session.user.id, data: t });
    });
    const deletedIds = Object.keys(prevMap).filter((id) => !(id in nextMap));
    prevSyncedRef.current = nextMap;

    if (upserts.length) {
      supabase.from(TRIPS_TABLE).upsert(upserts, { onConflict: "user_id,id" }).then(({ error }) => {
        if (error) console.error("Postmark: cloud save failed", error);
      });
    }
    deletedIds.forEach((id) => {
      supabase.from(TRIPS_TABLE).delete().eq("user_id", session.user.id).eq("id", id).then(({ error }) => {
        if (error) console.error("Postmark: cloud delete failed", error);
      });
    });
  }, [trips, loaded, cloudReady, session.user.id]);

  useEffect(() => {
    if (!styleLoaded) return;
    try { localStorage.setItem(STYLE_STORAGE_KEY, JSON.stringify(styleSettings)); } catch (e) { /* ignore */ }
    if (!cloudReady) return;
    const json = JSON.stringify(styleSettings);
    if (prevSyncedStyleRef.current === json) return;
    prevSyncedStyleRef.current = json;
    supabase.from(TRIPS_TABLE).upsert([{ id: STYLE_ROW_ID, user_id: session.user.id, data: styleSettings }], { onConflict: "user_id,id" }).then(({ error }) => {
      if (error) console.error("Postmark: style sync failed", error);
    });
  }, [styleSettings, styleLoaded, cloudReady, session.user.id]);

  function downloadBackup() {
    const payload = { app: "postmark", exportedAt: new Date().toISOString(), trips, styleSettings };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `postmark-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  function handleRestoreFile(e) {
    ...
    reader.onload = () => {
      try {
        ...
        const restoredTrips = Array.isArray(parsed.trips) ? parsed.trips.map(migrateTrip) : null;
        if (!restoredTrips) {
          window.alert("That file doesn't look like a Postmark backup.");
          return;
        }
        if (!window.confirm(`Restore ${restoredTrips.length} trip${restoredTrips.length === 1 ? "" : "s"} from this backup? This replaces everything currently in the app.`)) return;
        setTrips(restoredTrips);
        if (parsed.styleSettings) setStyleSettings({ ...DEFAULT_STYLE_SETTINGS, ...parsed.styleSettings });
        setActiveTripId(null);
      } catch (err) {
        window.alert("Couldn't read that file — is it a valid Postmark backup?");
      }
    };
  ...
  return (
    ...
    <div className="pm-root">
      ...
      {trips === null ? (
        ...
        <>
          <HeaderToolbar
            ...
            onRestoreClick={() => restoreFileRef.current && restoreFileRef.current.click()}
            session={session}
      ...
      <input ref={restoreFileRef} type="file" accept="application/json" onChange={handleRestoreFile} style={{ display: "none" }} />
...
function HeaderToolbar({ showSettingsGear, onOpenSettings, showBackupMenu, setShowBackupMenu, onDownloadBackup, onRestoreClick, session, cloudError }) {
  const email = (session && session.user && session.user.email) || "";
  const initial = email ? email[0].toUpperCase() : "?";

  return (
    <div style={{ position: "fixed", top: 16, right: 16, zIndex: 30, display: "flex", alignItems: "center", gap: 8 }}>
      {cloudError && (
        <span className="pm-mono" title="Couldn't reach cloud storage — using the local copy on this device." style={{ fontSize: 10, color: "var(--rust)", background: "#FFFDF9", border: "1.5px solid var(--rust)", borderRadius: 20, padding: "4px 8px" }}>
          offline
        </span>
      )}
      ...
      {showSettingsGear && (
        <button
          ...
          style={{ background: "#FFFDF9", border: "1.5px solid rgba(42,32,25,0.3)", borderRadius: "50%", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--ink)", boxShadow: "0 2px 6px rgba(0,0,0,0.15)" }}
        >
          <Settings size={16} />
        </button>
      )}
      ...
      <div style={{ position: "relative" }}>
        <button
          ...
          className="pm-mono"
          style={{ background: "#FFFDF9", border: "1.5px solid rgba(42,32,25,0.3)", borderRadius: 20, height: 34, padding: "0 12px", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", color: "var(--ink)", boxShadow: "0 2px 6px rgba(0,0,0,0.15)", fontSize: 12 }}
        >
          <Download size={13} /> Backup
        </button>
        {showBackupMenu && (
          <>
            <div onClick={() => setShowBackupMenu(false)} style={{ position: "fixed", inset: 0, zIndex: 39 }} />
            <div style={{ position: "absolute", top: 40, right: 0, background: "#FFFDF9", border: "1.5px solid rgba(42,32,25,0.2)", borderRadius: 10, boxShadow: "0 6px 20px rgba(0,0,0,0.2)", zIndex: 40, minWidth: 180, overflow: "hidden" }}>
              <button onClick={onDownloadBackup} style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--ink)" }}>
                <Download size={14} /> Download backup
              </button>
              <button onClick={onRestoreClick} style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--ink)", borderTop: "1px solid rgba(46,43,38,0.1)" }}>
      ...
      <div
        style={{ background: "#FFFDF9", border: "1.5px solid rgba(42,32,25,0.3)", borderRadius: 20, height: 34, padding: "0 6px 0 0", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 2px 6px rgba(0,0,0,0.15)" }}
      >
        <span
          title={email}
          style={{ width: 26, height: 26, borderRadius: "50%", background: "var(--forest)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, marginLeft: 3, flexShrink: 0 }}
        >
          {initial}
        </span>
        <button
          ...
          title="Sign out"
          className="pm-mono"
          style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink)", display: "flex", alignItems: "center", gap: 4, fontSize: 11, padding: "0 6px 0 0" }}
        >
          <LogOut size={13} />
        </button>
      </div>
...
function AuthLoadingScreen() {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F9F5E6", fontFamily: "'Space Mono', monospace", fontSize: 13, color: "#8A5C52" }}>
...
function AuthScreen() {
  ...
  const solidBtn = { width: "100%", justifyContent: "center", display: "flex", alignItems: "center", gap: 6, background: "#F0954B", color: "#702722", border: "1px solid #F0954B", borderRadius: 20, padding: "11px 16px", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "'Nunito', sans-serif" };

  return (
    <div style={{ minHeight: "100vh", background: "#F9F5E6", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bungee&family=Nunito:ital,wght@0,400;0,600;0,700;1,600&family=Space+Mono:wght@400;700&display=swap');
        .pm-auth-input { width: 100%; padding: 10px 12px; border-radius: 8px; border: 1.5px solid rgba(46,43,38,0.25); background: #FFFDF9; font-family: 'Nunito', sans-serif; font-size: 14px; outline: none; box-sizing: border-box; }
        .pm-auth-input:focus { border-color: #26422B; }
      `}</style>

      <div style={{ width: "100%", maxWidth: 360, background: "#FFFDF9", borderRadius: 16, padding: 32, boxShadow: "0 10px 30px rgba(0,0,0,0.15)", textAlign: isCheckEmail ? "center" : "left", fontFamily: "'Nunito', sans-serif", color: "#702722" }}>
        <div style={{ fontFamily: "'Bungee', serif", fontSize: 24, marginBottom: isCheckEmail ? 16 : 4 }}>Postmark</div>

        {isCheckEmail ? (
          <>
            <Mail size={40} style={{ color: "#26422B", margin: "8px auto 12px", display: "block" }} />
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Check your email</div>
            <div style={{ fontSize: 13, color: "#8A5C52", lineHeight: 1.6, marginBottom: 20 }}>
              We sent a confirmation link to <strong>{checkEmailAddress}</strong>. Click it, then come back here and sign in.
            </div>
            <button style={solidBtn} onClick={() => { setMode("signin"); setPassword(""); }}>Back to sign in</button>
          </>
        ) : (
          <>
            <div style={{ fontSize: 12, color: "#8A5C52", marginBottom: 20 }}>{mode === "signin" ? "Sign in to your trips" : "Create an account"}</div>
            <form onSubmit={mode === "signin" ? handleSignIn : handleSignUp} style={{ display: "grid", gap: 12 }}>
              <div>
                <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: "#8A5C52", marginBottom: 4 }}>EMAIL</div>
                <input className="pm-auth-input" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" />
              </div>
              <div>
                <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: "#8A5C52", marginBottom: 4 }}>PASSWORD</div>
                <input className="pm-auth-input" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete={mode === "signin" ? "current-password" : "new-password"} />
              </div>
              {error && <div style={{ fontSize: 12, color: "#D9421F" }}>{error}</div>}
            ...
            <button
              ...
              style={{ background: "none", border: "none", cursor: "pointer", color: "#8A5C52", fontSize: 12, marginTop: 16, textDecoration: "underline", padding: 0, fontFamily: "'Nunito', sans-serif" }}
            >
...
export default function App() {
  ...
  return <PostmarkApp key={session.user.id} session={session} />;
}
